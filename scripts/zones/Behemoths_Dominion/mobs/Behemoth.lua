...
function onMobDespawn(mob)
    ...
    if LandKingSystem_HQ ~= 1 and ToD <= os.time() and popNow then
        ...
        GetMobByID(ID.mob.KING_BEHEMOTH):setRespawnTime(600) -- 21 - 24 hours with half hour windows
    else
        if LandKingSystem_NQ ~= 1 then
            UpdateNMSpawnPoint(ID.mob.BEHEMOTH)
            GetMobByID(ID.mob.BEHEMOTH):setRespawnTime(600) -- 21 - 24 hours with half hour windows
